...
It's a detailed guide aimed at applying best practices in API manual testing, mainly using ad-hoc and exploratory testing approaches. It can be combined with sanity checks based on the provided api documentation and schema. No exact acceptance criteria are present. Creativity and exploring while sticking to the documentation are key to success in test case creation.
...
If `docs/api/` is empty, use **playwright-mcp** on `https://docs.inventree.org/en/stable/api/schema/part/` to load endpoint reference. Record which source was used.
...
Use **Read** on `/test-cases/${api}/{$suite_name}/references/tc-template.md` (in this skill's directory) to load the test case format.
