# TC-APFLT-017: GET /api/part/?purchasable=true&salable=true returns parts matching both flags
...
- Parts with both `purchasable=true` and `salable=true` exist on the server
- Valid auth token available

**Steps:**

1. Send `GET /api/part/?purchasable=true&limit=1` and record `count` as `PURCHASEABLE_COUNT`
2. Send `GET /api/part/?salable=true&limit=1` and record `count` as `SALABLE_COUNT`
3. Send `GET /api/part/?purchasable=true&salable=true&limit=5` with header `Authorization: Token <token>`
...
8. Verify every part in `results` has `purchasable` equal to `true`
...
- `purchasable=true` alone: would return a superset
- `salable=true` alone: would return a superset
- `purchasable=true&salable=true`: status `200`, count `38`, sample results: `pk=1934 "AllFlagsPart-TC004-1776159658932-dk4zc" purchasable: true salable: true`, `pk=2046 "AllFlagsPart-TC004-..." purchasable: true salable: true`, `pk=1161 "Buy-Sell Part 1776151541544" purchasable: true salable: true`
- All returned parts have both `purchasable: true` and `salable: true`
